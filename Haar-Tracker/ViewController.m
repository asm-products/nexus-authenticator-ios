...
- (void)viewDidLoad
{
    ...
    filename = [[NSBundle mainBundle]
                pathForResource:@"haarcascade_eye"
                ofType:@"xml"];
    parameters.eyesCascade1.load([filename UTF8String]);
    
    filename = [[NSBundle mainBundle]
                pathForResource:@"haarcascade_eye_tree_eyeglasses"
                ofType:@"xml"];
    parameters.eyesCascade2.load([filename UTF8String]);
...
{
    if(isCapturing)
        faceAnimator->addPerson();
}

-(IBAction)deleteAllButtonPressed:(id)sender
{
    if(isCapturing)
        faceAnimator->deleteAll();
}
